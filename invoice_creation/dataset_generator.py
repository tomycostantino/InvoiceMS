    ...
    for i in range(len(invoice_numbers)):
        ...
        invoices.append(
            {'full_name': full_name, 'city_postcode': city_postcode, 'country': country, 'reference_n': reference_n,
             'invoice_date': invoice_date, 'invoice_n': invoice_n, 'product_items': json.dumps(product_items),
...
def create_dataset(filename, n_rows):
    ivc_numbers = create_invoice_numbers(n_rows)
    invoices = create_invoices(ivc_numbers)
    write_csv(filename, invoices)


if __name__ == '__main__':
    create_dataset('dataset.csv', 200)
